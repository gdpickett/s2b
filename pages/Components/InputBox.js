...
function InputBox({ session }) {
    ...
    const sendPost = async (e) => {
        ...
        db.collection('posts').add({
            ...
            if (imageToPost) {
                const uploadTask = storage.ref(`images/${doc.id}`).putString(imageToPost, 'data_url')

                removeImage();

                uploadTask.on('state_change', null, (error) => console.error(error), () => {
                    storage.ref('images').child(doc.id).getDownloadURL().then(url => {
                ...
                */}
                ...
                {imageToPost &&
                    <div onClick={removeImage} className="flex flex-col filter hover:brightness-110 transition duration-150 transform hover: scale-105 cursor-pointer">
                        <Image src={imageToPost} alt="" className="object-contain h-10" width={40} height={40}/>
